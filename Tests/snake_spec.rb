...
RSpec.describe Snake do
  describe '#initialize' do
    it 'passing a player one id to Snake gets it passed to Button' do
      ...
      described_class.new(PlayerIds::PLAYER_ONE)
    ...
    it 'passing a player two id to Snake gets it passed to Button' do
      allow(Button).to receive(:new)
      described_class.new(PlayerIds::PLAYER_TWO)
    ...
    it 'making sure the color is set with the results from sampling the settings'do
      ...
      snake = described_class.new(PlayerIds::PLAYER_TWO)
    ...
    it 'making sure the color is set with the results from sampling the settings'do
      ...
      snake = described_class.new(PlayerIds::PLAYER_ONE)
      expect(snake.color_name).to eq('Red')
    end
    ...
    it'position is set for snake 1'do
      ...
      expected = [[xpos, Settings::GRID_HEIGHT - 3], [xpos, Settings::GRID_HEIGHT - 4],
                  [xpos, Settings::GRID_HEIGHT - 5]]
    ...
    it 'turned is initialized as false'do
      snake = described_class.new(PlayerIds::PLAYER_TWO)
      expect(snake.turned).to be(false)
    end
  ...
  describe 'hit_wall?'do
    it 'make sure the body has been called' do
      snake_1 = described_class.new(PlayerIds::PLAYER_ONE)
      snake_2 = described_class.new(PlayerIds::PLAYER_TWO)
    ...
    it 'crash is true' do
      ...
      expect(snake_1.hit_wall?(snake_2)).to be(true)
    end
    ...
    it 'crash is false' do
      ...
      expect(snake_1.hit_wall?(snake_2)).to be(false)
    end
  end
  ...
  describe('draw')do
    it 'draw_base is called 3 times'do
      snake = described_class.new(PlayerIds::PLAYER_ONE)
      call_count =0
      allow(snake).to receive(:draw_base) do | _arg|
        call_count +=1
      end
  ...
  describe 'draw_base tests'do
    ...
    it 'expect Square to be called with different arguments' do
      ...
      expect(Square).to have_received(:new).with(x: 3* Settings::GRID_SIZE, y: 5*Settings::GRID_SIZE,
                                                 size: Settings::NODE_SIZE, color: 'green', z: 0)
    end
  end
  ...
  describe 'new_direction tests'do
    ...
    it 'turned is true after execution' do
      ...
      snake.new_direction(Directions::DOWN)
      expect(snake.turned).to be(true)
    end
  end
